 ...
 * @Date: 7/29/2018
 * Sections marked TODO are completed by Author
...
public class MyFB {
	...
	static class Person {
		...
		public int hashCode () {
			...
			for(int i = 0; i < this.name.length(); i++) {
				result = this.age + this.name.charAt(i) + (41 * result);
			}
	...
	public boolean queryFriendship (Person p1, Person p2) {
		...
		if(map.containsKey(p1) && map.get(p1).contains(p2))
			return true;
	...
	public Iterable<Person> lookupFriends (Person p) {
		...
		if(map.containsKey(p)) {
			ArrayList<Person> it = new ArrayList<Person>();
			HashSet<Person> temp = map.get(p);
			for(Person current : temp) {
				it.add(current);
			}
			return it;	
		}
	...
	public static void main (String[] args) {
		...
		//[] inputFiles = new In[] { new In () }; // from console
